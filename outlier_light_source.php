...
<table border="1">
    ...
    <tr>
        <th>Outlier</th>
            ...
            <td>
                <?php 
                    echo "Total outlier : " . $total_outlier_light;
                ?>
            </td>
    </tr>
    <tr>
            ...
            <td>
                <table border="1" class="data-outlier">
                    <?php 
                        ...
                        $no = $limit_start + 1;
                        // OUTLIER 
                        $query = "SELECT light FROM datatraining WHERE light<$lower_limit OR light>$upper_limit LIMIT $limit_start, $limit";
            ...
            <td>
                <nav class="mt-1">
                    <ul class="m-0 pagination font-pagination">
                        <?php
                            // OUTLIER PAGINATION
                            $jumlah_page = ceil($total_outlier_light / $limit);
                            $jumlah_number = 1;
                            $start_number = ($page > $jumlah_number) ? $page - $jumlah_number : 1;
                            $end_number = ($page < ($jumlah_page - $jumlah_number)) ? $page + $jumlah_number : $jumlah_page;

                            if ($total_outlier_light > $limit) {
